...
export default function GoogleAccountScreen() {
  ...
  const [isLinked, setIsLinked] = useState(true); // Mock real state from auth provider
  ...
  return (
    <SafeAreaView edges={["top"]} style={{ flex: 1, backgroundColor: theme.colors.background }}>
      <View style={styles.header}>
        ...
        <Text style={[styles.headerTitle, { color: theme.colors.textPrimary }]}>Google Account</Text>
        <View style={{ width: 40 }} />
      </View>
      ...
      <ScrollView contentContainerStyle={{ padding: 24 }}>
        <View style={styles.centerSection}>
          <View style={[styles.iconWrapper, { backgroundColor: theme.colors.surfaceElevated }]}>
            <Ionicons name="logo-google" size={48} color={theme.colors.textPrimary} />
          </View>
        ...
        {isLinked ? (
          <View style={[styles.infoCard, { backgroundColor: theme.colors.surfaceElevated }]}>
            <Text style={[styles.infoLabel, { color: theme.colors.textSecondary }]}>Google Account Email</Text>
            <Text style={[styles.infoValue, { color: theme.colors.textPrimary }]}>{user?.email || "linked.account@gmail.com"}</Text>
          </View>
        ) : (
          <Text style={[styles.descText, { color: theme.colors.textSecondary }]}>
            Connect your Google account to easily sign in without remembering another password.
          </Text>
        )}
        ...
        >
          {isLoading ? (
            ...
            <Text style={[styles.primaryBtnText, { color: isLinked ? "#FF5C5C" : "#FFF" }]}>
              {isLinked ? "Unlink Google Account" : "Connect Google Account"}
            </Text>
          )}
        </TouchableOpacity>
      </View>
